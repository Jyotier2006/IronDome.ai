...
from threat_ip_ledger import ip_manager, BlockReason, ThreatSeverity
...
# Demo Control Panel (frontend) broadcasts synthetic telemetry/alerts to
# preview the UI without a real attack. Echo them to all clients so the
# dashboard reacts the same way it would to a real backend-sourced event.
@sio.on('telemetry')
async def client_telemetry(sid, data):
    await sio.emit('telemetry', data)

@sio.on('alert')
async def client_alert(sid, data):
    await sio.emit('alert', data)

@sio.on('device:status')
async def client_device_status(sid, data):
    await sio.emit('device:status', data)

@sio.on('ip:dropped')
async def client_ip_dropped(sid, data):
    await sio.emit('ip:dropped', data)

@sio.on('ip:blocked')
async def client_ip_blocked(sid, data):
    await sio.emit('ip:blocked', data)

@sio.on('ip:rate_limited')
async def client_ip_rate_limited(sid, data):
    await sio.emit('ip:rate_limited', data)
...
# Rule IDs from the telemetry-based detection engine that represent a
# genuinely blocked attack, mapped to the Dropped Packets panel's attack types.
# These alerts never pass through IPBlockingMiddleware (they're derived from
# analyzing ingested events, not live HTTP requests), so without this they'd
# show up in Alerts but never in Dropped Packets.
DROPPED_RULE_MAP = {
...
def _extract_evidence_ip(evidence: dict) -> str:
    if not evidence:
        return "unknown"
    for key in ("source_ip", "sourceIP", "ip", "attacker_ip"):
...
async def receive_alert(alert: AlertEvent):
    try:
        ...
        attack_type = DROPPED_RULE_MAP.get(alert.rule_id)
        if attack_type:
            evidence = alert.evidence or {}
            source_ip = _extract_evidence_ip(evidence)
            endpoint = evidence.get("path") or evidence.get("targetEndpoint") or evidence.get("endpoint") or "/unknown"
            severity = alert.severity if alert.severity in ("low", "medium", "high", "critical") else "high"
            ...
            if attack_type == "sql_injection":
                ip_manager.record_sql_injection_blocked(
                    ...
                    endpoint=endpoint,
                    method="POST",
                    pattern=str(evidence.get("query") or evidence.get("payload") or "pattern match"),
                    severity=severity,
                )
            elif attack_type == "brute_force":
                ip_manager.record_brute_force_blocked(
                    ip=source_ip,
                    endpoint=endpoint,
                    method="POST",
                ...
                ip_manager.record_flooding_blocked(
                    ip=source_ip,
                    endpoint=endpoint,
                    method="GET",
                    rate=evidence.get("requestsPerSecond") or evidence.get("request_count", 0),
                    severity=severity,
                )
